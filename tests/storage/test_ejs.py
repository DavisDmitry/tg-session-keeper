...
from session_keeper.storage.ejs import FileEncryptedJsonStorage as EncryptedJsonStorage
...
async def storage(temp_file: str, api_id: int, api_hash: str) -> EncryptedJsonStorage:
    storage = EncryptedJsonStorage(PASSWORD, temp_file)
...
async def test_encrypt_and_decrypt(storage_with_session: EncryptedJsonStorage):
    ...
    async with EncryptedJsonStorage(PASSWORD, filename) as storage2:
